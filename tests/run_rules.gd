...
func run() -> void:
	...
	var owned_test: Array[StringName] = [&"anchor_coil", &"stabilizing_sight"]
	var offer_test: Array[StringName] = RunRules.draw_offers(owned_test, offer_test_rng)
	_check(offer_test.size() == 3 and not offer_test.has(&"anchor_coil") and not offer_test.has(&"stabilizing_sight") and offer_test[0] != offer_test[1] and offer_test[1] != offer_test[2], "offers exclude owned modules and duplicates")
